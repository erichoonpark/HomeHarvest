...
def test_raw_details_private_signal_overrides_canonical_unknown():
    module = _load_module()
    assumptions = _assumptions()
    ...
    df = pd.DataFrame(
        [
            {
                "property_id": "RAW_OVERRIDES_CANONICAL",
                "status": "FOR_SALE",
                "street": "500 Main St",
                "city": "Palm Springs",
                "state": "CA",
                ...
                "sqft": 2200,
                "str_nbhd_under_cap_current": 1,
                "property_url": "https://example.com/raw-overrides-canonical",
                "is_private_pool": False,
                "is_private_pool_known": False,
    ...
    assert bool(row["is_private_pool"]) is True
    assert bool(row["is_private_pool_known"]) is True
    assert row["has_pool_source"] == "raw_details"
    assert bool(row["str_fit_pass"]) is True
